      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="">
            ...
            <Input type="text" id="client-name" placeholder="Ex: Família Silva" value={formData.clientName} onChange={handleInputChange} className="w-full bg-white" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input type="text" id="service-value" placeholder="R$ 0,00" value={formData.serviceValue} onChange={handleInputChange} className="w-full bg-white" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn("w-full justify-start text-left font-normal bg-white", !formData.serviceDate && "text-muted-foreground")}>
                  <Calendar className="mr-2 h-4 w-4" />
                  {formData.serviceDate ? format(new Date(formData.serviceDate), "dd/MM/yyyy") : "Selecione uma data"}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-white" align="start">
                <CalendarComponent mode="single" selected={formData.serviceDate ? new Date(formData.serviceDate) : undefined} onSelect={handleDateSelect} initialFocus className="p-3 pointer-events-auto" />
              </PopoverContent>
            </Popover>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.paymentMethod} onValueChange={value => handleSelectChange(value, 'paymentMethod')}>
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.paymentStatus} onValueChange={value => handleSelectChange(value, 'paymentStatus')}>
        ...
        <DialogFooter>
          <button type="button" onClick={onClose} className="px-4 py-2 border border-border rounded-lg text-sm font-medium hover:bg-muted transition-colors">
            Cancelar
          </button>
          <button type="button" onClick={handleSave} className="premium-button flex items-center space-x-2">
